...
export function NavigationImage({onAdd}: any) {
    const onImageLoad = (files: FileList | null) => {
        if (!files) {
            return;
        }
    ...
    return (
            <a onClick={onAdd} href="#" className="bar_item">Изображение
                ...
                <label className="label_add-image">
                    <input onChange={(event) => onImageLoad(event.target.files)} style={{display: 'none'}} accept=".jpg, .png" type="file" id="file_button"></input>
                </label>
            </a>
    )
}